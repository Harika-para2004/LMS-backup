...
const AdminAnalytics = ({year}) => {
  ...
  const fetchAnalytics = async (year) => {
    ...
    try {
      const [trendsRes, typesRes, deptRes, topEmployeesRes, leaveStatusRes] =
        await Promise.all([
          axios.get(`${backendUrl}/admin/leave-trends/${year}`),
          axios.get(
            `${backendUrl}/admin/leave-type-distribution/${year}`
          ),
          axios.get(`${backendUrl}/admin/department-leave/${year}`),
          axios.get(`${backendUrl}/admin/top-leave-takers/${year}`),
          axios.get(`${backendUrl}/admin/leave-status/${year}`),
        ]);
  ...
  const fetchManagers = async () => {
    try {
      const res = await axios.get(`${backendUrl}/admin/managers`);
  ...
  useEffect(() => {
    if (selectedManager) {
      axios
        .get(
          `${backendUrl}/admin/manager-leave-trends/${year}/${selectedManager}`
        )
